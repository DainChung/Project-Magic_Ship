...
public class EnemyAI : MonoBehaviour
{
    ...
    private EnemyAIEngine _enemyAIEngine;
    private UnitBaseEngine _unitBaseEngine;
    ...
    public void AI_Simple_Level0()
    {
        ...
        if (!isEnd_OF_Stage)
        {
            ...
            if (enemyController.enemyStat.__PUB__Health_Point <= enemyController.enemyStat.half_HP)
            {
                //플레이어 반대 방향을 보도록 한다.
                _enemyAIEngine.Rotate_TO_Direction(enemyController.enemyStat.__PUB_Rotation_Speed, ref enemyController.enemyTransform, enemyController.playerTransform, true);

                //일정 시간동안 해당 유닛의 전방을 향해 이동한다.
                if (_enemyAIEngine._unitBaseEngine.CoolTimeCheck(0))
                {
                    _enemyAIEngine._unitBaseEngine._unit_Move_Engine.Move_OBJ(enemyController.enemyStat.__PUB_Move_Speed, ref enemyController.enemyTransform, 1);
                    //4초 동안 퇴각
                    StartCoroutine(enemyController.enemyCoolTimer.Timer_Do_Once(4.0f, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[0] = input; }, true));
                }
                else
                {
                    //16초 동안 정지
                    StartCoroutine(enemyController.enemyCoolTimer.Timer_Do_Once(16.0f, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[0] = input; }, false));
                }
                
                if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[0])
                {
                    _enemyAIEngine._unitBaseEngine._unit_Move_Engine.Move_OBJ(enemyController.enemyStat.__PUB_Move_Speed, ref enemyController.enemyTransform, 1);
                    //4초 동안 퇴각
                    StartCoroutine(enemyController.enemyCoolTimer.Timer_Do_Once(4.0f, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[0] = input; }, true));
            ...
            else
            {
                ...
                //플레이어를 바라보도록 한다.
                _enemyAIEngine.Rotate_TO_Direction(enemyStat.__PUB_Rotation_Speed, ref enemyController.enemyTransform, enemyController.playerTransform, false);

                //전방으로 이동한다.
                _enemyAIEngine.Go_TO_Foward_UNTIL_RayHit(enemyStat.__PUB_Move_Speed, ref enemyController.enemyTransform, enemyController.playerTransform);

                //각도 차에 따라 다른 위치로 발사한다. (아직 미구현)
                if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1])
                {
                    ...
                    _enemyAIEngine.Attack_Default(sampleTime, ref enemyController.enemy_Front, enemyStat, 1);
                    ...
                    //{
                    //    _enemyAIEngine._unitBaseEngine._unit_Combat_Engine.Using_Skill(ref enemyController.enemy_Front, sampleSkillTest, true);
                    ...
                    //_enemyAIEngine.Attack_Default(2.0f + Random.Range(0.0f, 1.0f), ref enemyController.enemy_Left, enemyStat, 1);
                    //플레이어가 적의 우측에 있을 때
                    //else if(  curAngleComparison <= 100.0f && curAngleComparison >= 80.0f && !( GET_RotataionDir(curAngle, destiAngle) )  )
                    //_enemyAIEngine.Attack_Default(2.0f + Random.Range(0.0f, 1.0f), ref enemyController.enemy_Right, enemyStat, 1);
                }
                //측면 공격
                //if (_unitBaseEngine._PUB_enemy_Is_ON_CoolTime[2])
                //{
                //    _unitBaseEngine.Attack_Default(2.0f, ref default_Ammo, ref enemy_Right, enemyStat.__PUB_ATK__Val, 1);
                //}
                //if (_unitBaseEngine._PUB_enemy_Is_ON_CoolTime[3])
                //{
                //    _unitBaseEngine.Attack_Default(2.0f, ref default_Ammo, ref enemy_Left, enemyStat.__PUB_ATK__Val, 2);
                //}
            }         
        }
        //스테이지 경계선과 접촉했을 때
        ...
        //20190607 임시
        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[8])
        {
            behaveCount_FOR_PPT++;
            ...
            hitCounter = 0;
            StartCoroutine(enemyController.enemyCoolTimer.Timer(sampleTime, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[8] = input; }, true));
    ...
    public void AI_Simple_Level0_WITH_BOSS()
    {
        ...
        if (!isEnd_OF_Stage)
        {
            ...
            if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1])
            {
                ...
                //if(curAngleComparison <= 10.0f)
                //쿨타임에 랜덤변수를 더해서 난이도를 조금 올린다.
                _enemyAIEngine.Attack_Default(2.0f + Random.Range(0.0f, 1.0f), ref enemyController.enemy_Front, enemyStat, 1);
        ...
        if (!isEnd_OF_Stage)
        {
            ...
            if (enemyController.enemyStat.__PUB__Health_Point > enemyController.enemyStat.half_HP)
            {
                ...
                if (behaveIndex == 0)
                {
                    ...
                    //정면 공격 수행
                    if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[2])
                    {
                        _enemyAIEngine.Attack_Default(2.0f + Random.Range(0.0f, 1.0f), ref enemyController.enemy_Front, enemyStat, 2);
                        behaveCounter++;
                    }
                ...
                else if (behaveIndex == 1)
                {
                    ...
                    if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1])
                    {
                        _enemyAIEngine._unitBaseEngine._unit_Move_Engine.Move_OBJ(enemyStat.__PUB_Move_Speed, ref enemyController.enemyTransform, 1);
                        StartCoroutine(transform.GetComponent<UnitCoolTimer>().Timer_Do_Once(2.0f, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1] = input; }, true));
                    }
                    else
                    {
                        //플레이어를 바라보도록 한다.
                        _enemyAIEngine.Rotate_TO_Direction(enemyStat.__PUB_Rotation_Speed, ref enemyController.enemyTransform, enemyController.playerTransform, false, enemyController.enemy_Right);

                        //충분히 가까워질 때까지 느리게 전방으로 이동한다.
                        _enemyAIEngine.Go_TO_Foward_UNTIL_RayHit(enemyStat.__PUB_Move_Speed * 0.3f, ref enemyController.enemyTransform, enemyController.playerTransform);

                        //우현 기본 공격
                        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[2] && !(_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[3]))
                        {
                            _enemyAIEngine.Attack_Default(2.0f + Random.Range(0.0f, 1.0f), ref enemyController.enemy_Right, enemyStat, 2);
                        }
                        //우현 산탄 스킬
                        else if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[3])
                        {
                            EnemyUsingSkill(3, enemyController.enemy_Right, enemyStat._GET_enemySkillList[0]);
                            behaveCounter++;
                        }

                        //우현 산탄 스킬 3회 시전 후 행동 패턴을 behave == 0으로 변경, behaveCounter 초기화, 일정 시간동안 전방이동 bool 초기화
                        if (behaveCounter >= 3) { behaveIndex = 2; behaveCounter = 0; _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1] = true; }
                    }
                }
                ...
                else if (behaveIndex == 2)
                {
                    ...
                    {
                        //플레이어를 바라보도록 한다.
                        _enemyAIEngine.Rotate_TO_Direction(enemyStat.__PUB_Rotation_Speed, ref enemyController.enemyTransform, enemyController.playerTransform, false, enemyController.enemy_Left);
                        ...
                        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[2] && !(_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[3]))
                        {
                            _enemyAIEngine.Attack_Default(2.0f + Random.Range(0.0f, 1.0f), ref enemyController.enemy_Left, enemyStat, 2);
                        }
                        //좌현 산탄 스킬
                        else if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[3])
                        {
                            EnemyUsingSkill(3, enemyController.enemy_Left, enemyStat._GET_enemySkillList[0]);
                            behaveCounter++;
                        }

                        //좌현 산탄 스킬 3회 시전 후 행동 패턴을 behave == 0으로 변경, behaveCounter 초기화, 일정 시간동안 전방이동 bool 초기화
                        if (behaveCounter >= 3) { behaveIndex = 0; behaveCounter = 0; _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1] = true; }
                    }
                }
                else
                {
            ...
            {
                //Debug.Log(_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[4]);
                ...
                else
                {
                    if (behaveIndex != 4)
                    {
                        ...
                        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[4] && enemyStat.__PUB__Power_Point < enemyStat.__GET_Max_PP)
                        {
                            //게이지를 모은다.
                            EnemyUsingSkill(4, enemyController.enemy_Left, enemyStat._GET_enemySkillList[1]);
                        }
                        //게이지 모을 때까지 쿨타임이 남았다면
                        else if (!_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[4] && enemyStat.__PUB__Power_Point < enemyStat.__GET_Max_PP)
                        {
                            //도망친다.
                            _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[0] = true;
                        }
                        //궁극기 게이지가 충분하고 궁극기를 사용할 수 있으면
                        else if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[5] && enemyStat.__PUB__Power_Point == enemyStat.__GET_Max_PP)
                        {
                            //플레이어와 일정거리 이하일 때
                            if (Vector3.Distance(transform.position, enemyController.playerTransform.position) <= 10f)
                            {
                                //궁극기를 사용한다.
                                EnemyUsingSkill(5, enemyController.enemy_Front, enemyStat._GET_enemySkillList[2]);
                                StartCoroutine(enemyController.enemyCoolTimer.Timer(enemyStat._GET_enemySkillList[2].__GET_Skill_ING_Time, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[6] = input; }, true));
                                behaveIndex = 4;
                            }
                            //플레이어와 일정거리 이상일 때
                            else
                            {
                                //플레이어에게 접근한다.
                                _enemyAIEngine.Rotate_TO_Direction(enemyController.enemyStat.__PUB_Rotation_Speed, ref enemyController.enemyTransform, enemyController.playerTransform, false);
                    ...
                    else if (behaveIndex == 4)
                    {
                        ...
                        if (!(enemyController.playerTransform.GetComponent<PlayerController>()._GET_isPlayerHitUltimateSkill) || _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[6])
                        {
                            //궁극기 게이지를 모으도록 한다.
                            behaveIndex = 0;
                            //행동 bool 변수 초기화
                            _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[0] = true;
                            _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[6] = true;
                        }
                        //플레이어가 궁극기에서 못 벗어나면
                        else
                        {
                            ...
                            {
                                EnemyUsingSkill(3, enemyController.enemy_Front, enemyStat._GET_enemySkillList[0]);
                            }
                        }
                    }
                }
    ...
    void Go_INTO_Stage()
    {
        ...
        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1])
        {
            _enemyAIEngine.Rotate_TO_Direction(enemyStat.__PUB_Rotation_Speed, ref enemyController.enemyTransform, middle_OF_Stage, false);

            _enemyAIEngine._unitBaseEngine._unit_Move_Engine.Move_OBJ(enemyStat.__PUB_Move_Speed, ref enemyController.enemyTransform, -1);

            StartCoroutine(enemyController.enemyCoolTimer.Timer_Do_Once(3.0f, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1] = input; }, true));
        }
        //3초 동안 전방으로 이동한다.
        else
        {
            _enemyAIEngine._unitBaseEngine._unit_Move_Engine.Move_OBJ(enemyStat.__PUB_Move_Speed, ref enemyController.enemyTransform, 1);
            StartCoroutine(enemyController.enemyCoolTimer.Timer_Do_Once(3.0f, (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1] = input; }, false));
        }
    }
    ...
    {
        //쿨타임이 지났을 때
        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[index])
        {
            //마나가 충분할 때
            if (enemyStat.__Is_Mana_Enough(whichSkill))
            {
                //필요한 마나 또는 궁극기 게이지 소모
                if (whichSkill.__GET_Skill_Code_M != SkillBaseCode._SKILL_CODE_Main.FIN) enemyStat.__Get_HIT__About_Mana(whichSkill.__GET_Skill_Use_Amount, 1);
                else enemyStat.__Get_HIT__About_Power(whichSkill.__GET_Skill_Use_Amount, 1);

                //UnitBaseEngine.Using_Skill에서 스킬 기능 처리
                _enemyAIEngine._unitBaseEngine._unit_Combat_Engine.Using_Skill(ref enemyAttacker, whichSkill, true);
                //쿨타임 관련 처리
                StartCoroutine(
                    enemyController.enemyCoolTimer.Timer(whichSkill.__GET_Skill_Cool_Time,
                    (input) => { _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[index] = input; }, _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[index])
                    );
            }
            //마나가 부족할 때
            else
    ...
    public void AI_DeepLearning__Random_Ver()
    {
        ...
        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1] && realIndex.vecZ != 0)
        {
            //주어진 공격 수행
    ...
    public void AI_DeepLearning__BigData_Ver()
    {
        ...
        if (sitCUR._doing.vecZ != 0)
        {
            if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1])// && sitCUR._doing.vecZ < behaveList_Attack.Count && curDist < 30f)
            {
                //주어진 공격 수행
    ...
    public void AI_DeepLearning_NoFeedback()
    {
        ...
        if (sitCUR._doing.vecZ != 0 && _enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1])
        {
            //검색한 공격 행동 수행
            behaveList_Attack[sitCUR._doing.vecZ]();
        }
    ...
    public void __OLD__AI_DeepLearning__BigData_Ver()
    {
        ...
        //1차적으로 일반 공격만 생각하도록 한다.
        //공격 쿨타임도 끝났고 공격을 하려는 경우
        if (_enemyAIEngine._PUB_enemy_Is_ON_CoolTime[1] && sitCUR._doing.vecZ != 0 && sitCUR._doing.vecZ < behaveList_Attack.Count)
        {
            //주어진 공격 수행
            behaveList_Attack[sitCUR._doing.vecZ]();
        }
    ...
    public void AI_ReinforceLearn_RandomBehave_Ver()
    {
        ...
        if (isbehaveCoolTimeOn)
        {
            ...
            //if (curDist > 50f && enemyController.enemyStat.__PUB__Health_Point > 5)
            //    behaveScore -= (int)((curDist - 50) / 10);
            //else if (curDist > 50f && enemyController.enemyStat.__PUB__Health_Point <= 5)
            //    behaveScore += (int)((curDist - 50) / 10);
            ...
            //모든 데이터 통합 관리
            //if (enemyController.enemyStat.__PUB__Health_Point <= 5)
            //{
